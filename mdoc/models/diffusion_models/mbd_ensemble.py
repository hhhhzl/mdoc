...
class ModelBasedDiffusionEnsemble(nn.Module):
    ...
    def _rollout_single_batch_new2(self, model_id, state_q, us, constraints):
        ...
        v_err = u_norm - v_bar
        runtime_cost.add_(v_err.squeeze(-1) * v_err.squeeze(-1), alpha=0.5)
    ...
    def run_inference(
        ...
        for model_id in self.models.keys():
            ...
            self.constraints_ro = pack_constraints_dict(self.soft_constraints, M_max=1, N_max=15, D=2, H=64, device=self.device)
            # Reverse
            traj_i = traj_n
            model_chain = []
            costs = []
