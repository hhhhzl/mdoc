...
class ModelBasedDiffusionEnsemble(nn.Module):
    ...
    def __init__(
        ...
        self.compile_constraints = compile and compile_constraints
        self._compiled_pack_fns: Dict[Tuple[int, int, int, int], Callable[[dict[int, list]], Tuple[torch.Tensor, torch.Tensor, torch.Tensor]]] = {}
        self._rollout_accs: Dict[int, "RolloutAccelerator"] = {}
    ...
    def _apply_constraints(
            ...
            v_seq: torch.Tensor,
            a_safe: torch.Tensor,
            constraints: ConstraintTensor,
    ) -> torch.Tensor:
        """
        Apply CBF and vertex constraints in eager mode using the latest trajectory estimate.
        """
        B, H, _ = a_safe.shape
        ...
        offset = self.transforms[model_id].to(device=device, dtype=dtype).view(1, 1, 2) if has_tf else None

        q_ws_seq = q_seq + offset if has_tf else q_seq
        sdf_seq, grad_seq = self.sample_sdf_and_grad(q_ws_seq, model_id=model_id)
        ...
        h = sdf_seq - (R + margin0)
        A = grad_seq.view(B * H, 2)
        a_flat = a_safe.reshape(B * H, 2)
        v_flat = v_seq.reshape(B * H, 2)
        h_flat = h.reshape(B * H)

        Anorm = A.norm(dim=-1)
        goodA = Anorm > 1e-9
        n = torch.zeros_like(A)
        n[goodA] = A[goodA] / (Anorm[goodA].unsqueeze(-1) + 1e-12)

        hdot = (n * v_flat).sum(dim=-1)
        k0, k1 = 1.0, 4.0
        b = -(k1 * hdot + k0 * h_flat)
        need = goodA & (h_flat < tau)

        lhs = (n * a_flat).sum(dim=-1)
        den = (n * n).sum(dim=-1).add_(1e-9)
        delta = ((b - lhs).clamp_min(0.0) / den) * need.to(dtype)
        a_flat = a_flat + delta.unsqueeze(-1) * n

        lhs2 = (n * a_flat).sum(dim=-1)
        bad = goodA & (lhs2 < b)
        delta2 = ((b - lhs2).clamp_min(0.0) / den) * bad.to(dtype)
        a_flat = a_flat + delta2.unsqueeze(-1) * n

        still_in = goodA & (h_flat < 0.0)
        if still_in.any():
            step = (-h_flat[still_in] * 0.5).clamp_min(0.0).clamp_max(0.10)
            q_ws_flat = q_ws_seq.reshape(B * H, 2).clone()
            q_ws_flat[still_in] = q_ws_flat[still_in] + step.unsqueeze(-1) * n[still_in]
            q_ws_seq = q_ws_flat.view(B, H, 2)
            if has_tf:
                q_seq = q_ws_seq - offset
            else:
                q_seq = q_ws_seq

        a_safe = a_flat.view(B, H, 2)

        C_all = constraints.C2[model_id].to(device=device, dtype=dtype).view(-1, 2)
        R_all = constraints.R[model_id].to(device=device, dtype=dtype).view(1, -1)
        TM_all = constraints.TM[model_id]

        if C_all.numel() > 0 and TM_all.numel() > 0:
            TM = TM_all.view(-1, H).T.to(device=device)
            q_ws_seq = q_seq + offset if has_tf else q_seq

            diff = q_ws_seq.unsqueeze(2) - C_all.view(1, 1, -1, 2)
            dist = diff.norm(dim=-1).clamp_min_(1e-6)
            A_s = diff / dist.unsqueeze(-1)
            r_exp = (R_all + (R + 0.02))
            h_s = dist - r_exp

            active = TM.view(1, H, -1).expand(B, -1, -1)
            ...
            if near.any():
                ...
                idx = topk.indices
                idx_e = idx.unsqueeze(-1).expand(-1, -1, -1, 2)

                sel_A = torch.gather(A_s, 2, idx_e)
                sel_h = torch.gather(h_s, 2, idx)
                den_k = (sel_A * sel_A).sum(dim=-1).add_(1e-9)

                v_rep = v_seq.unsqueeze(2).expand(-1, -1, K, -1)
                hdot_k = (sel_A * v_rep).sum(dim=-1)
    ...
    def _pack_constraints_tensor(
        ...
        if not self.compile_constraints:
            return pack_constraints_dict(
                constraints,
                M_max=M_max,
                N_max=N_max,
                D=D,
        ...
        key = (M_max, N_max, D, H)
        fn = self._compiled_pack_fns.get(key)

        if fn is None:
            def _wrapper(constraints_in: dict[int, list]):
                packed = pack_constraints_dict(
                    constraints_in,
                    M_max=M_max,
                    N_max=N_max,
                    D=D,
                ...
                return packed.C2, packed.R, packed.TM

            try:
                fn = torch.compile(_wrapper, fullgraph=False, dynamic=True)
            except Exception as exc:
                print(f"[WARN] Failed to compile pack_constraints_dict: {exc}")
                self.compile_constraints = False
                return pack_constraints_dict(
                    constraints,
                    M_max=M_max,
                    N_max=N_max,
            ...
            self._compiled_pack_fns[key] = fn

        C2, R, TM = fn(constraints)
        return ConstraintTensor(C2=C2, R=R, TM=TM)

    def _get_rollout_acc(
            self,
            model_id: int,
            *,
            use_amp: bool,
        ...
        acc = self._rollout_accs.get(model_id)
        if acc is not None:
            return acc

        from mdoc.utils.accelerators import RolloutAccelerator

        params = self.models[model_id]['params']
        B = params['n_samples']
        H = params['horizon']

        example_actions = torch.zeros(B, H, 2, device=self.device, dtype=torch.float32)
        example_state = self.state_inits.q.to(device=self.device, dtype=torch.float32)
        example_constraints = self._pack_constraints_tensor(
            {mid: [] for mid in self.models.keys()},
            M_max=len(self.models),
            N_max=15,
            D=self.robot.q_dim,
            H=H,
            device=self.device,
        ...
        def _rollout_fn(state_q: torch.Tensor, actions: torch.Tensor, constraints: ConstraintTensor):
            return self._rollout_full(model_id, state_q, actions, constraints)

        acc = RolloutAccelerator(
            fn=_rollout_fn,
            example_args=(example_state, example_actions, example_constraints),
            use_amp=use_amp,
            amp_dtype=amp_dtype,
        ...
        acc.prepare()
        self._rollout_accs[model_id] = acc
        return acc
    ...
    def _compute_costs(
            ...
            v_seq: torch.Tensor,
            a_safe: torch.Tensor,
            sdf_penalty: torch.Tensor,
    ) -> Tuple[torch.Tensor, torch.Tensor]:
        """
        Compute runtime and terminal costs given the safe trajectory.
        """
        device = q_seq.device
        dtype = q_seq.dtype

        B, H, _ = q_seq.shape
        dt = a_safe.new_tensor(float(self.robot.dt))

        q_goal = goal_q.to(device=device, dtype=dtype).view(1, 1, 2)
        dist_to_goal = (q_seq - q_goal).norm(dim=2)
        ctrl = (a_safe * a_safe).sum(dim=2)

        runtime_cost = torch.zeros(B, H, device=device, dtype=dtype)
        runtime_cost.add_(ctrl, alpha=1.0)
        runtime_cost.add_(dist_to_goal, alpha=5.0)

        t_star = max(1, H - 1)
        t_idx = torch.arange(H, device=device, dtype=dtype).view(1, H)

        d0 = (state_q.to(device=device, dtype=dtype) - goal_q.to(device=device, dtype=dtype)).norm().view(1, 1)
        d_hat = d0 * torch.clamp(1.0 - t_idx / t_star, min=0.0)
        runtime_cost.add_((dist_to_goal - d_hat).pow(2), alpha=8.0)

        s = (t_idx / t_star).clamp(max=1.0).view(1, H, 1)
        v_bar = (d0.view(1, 1, 1) / (t_star * dt + 1e-6)) * (s * (1.0 - s))
        v_norm = v_seq.norm(dim=-1, keepdim=True)
        runtime_cost.add_((v_norm - v_bar).squeeze(-1).pow(2), alpha=1.0)

        r_goal = 0.10
        inside_goal_early = (dist_to_goal <= r_goal) & (t_idx < t_star)
        tau_e = ((t_star - t_idx).clamp(min=0).float() / H).to(dtype)
        runtime_cost.add_(tau_e * inside_goal_early.float(), alpha=0.5)

        runtime_cost.add_(5000.0 * sdf_penalty.pow(2))

        terminal_w = getattr(self, "w_T_base", 8)
        terminal = terminal_w * dist_to_goal[:, -1]
        total_cost = runtime_cost.sum(dim=1) + terminal

        return total_cost, runtime_cost
    ...
    def _rollout_full(
        ...
        state_q = state_q.to(device)
        q0 = state_q.view(1, 1, 2).expand(B, 1, 2)
        ...
        n = A / (Anorm.view(-1, 1) + 1e-12)
        step = (-h * 0.5).clamp_min(0.0).clamp_max(0.10)
        q_ws_flat_new = q_ws_flat + step.unsqueeze(-1) * n
        mask_si = (h < 0.0).view(-1, 1)
        blended = (
            torch.where(mask_si, q_ws_flat_new - offset, q_init_exp)
            if has_tf
            else torch.where(mask_si, q_ws_flat_new, q_init_exp)
        )
        ...
        C_all = constraints.C2[model_id].view(-1, 2)
        R_all = constraints.R[model_id].view(1, -1)
        TM_all = constraints.TM[model_id]

        if C_all.numel() > 0:
            q_ws_seq = q_seq + offset.view(1, 1, 2) if has_tf else q_seq
            diff = q_ws_seq.unsqueeze(2) - C_all.view(1, 1, -1, 2)
            dist = diff.norm(dim=-1).clamp_min_(1e-6)
            A_s = diff / dist.unsqueeze(-1)
            r_exp = R_all + (R + 0.02)
            ...
            if K > 0:
                ...
                sel_A = torch.gather(A_s, 2, idx_e)
                sel_h = torch.gather(h_s, 2, idx)
                sel_mask = torch.gather(near, 2, idx)

                beta = 0.05 if getattr(self, "agent_id", 0) % 2 == 0 else -0.05
                sel_b = -(eta / dt) * sel_h + beta

                lhs_k = (sel_A * u_safe.unsqueeze(2)).sum(dim=-1)
                need_k = sel_mask & (lhs_k < sel_b)
                den_k = (sel_A * sel_A).sum(dim=-1).add_(1e-9)
                alpha_k = torch.where(
                    need_k,
                    ((sel_b - lhs_k).clamp_min(0.0) / den_k),
                    torch.zeros_like(lhs_k),
                )
                delta_k = (alpha_k.unsqueeze(-1) * sel_A).sum(dim=2)
                u_safe = u_safe + delta_k

                q_seq = q0 + torch.cumsum(u_safe * dt, dim=1)

        q_ws = q_seq + offset.view(1, 1, 2) if has_tf else q_seq
        sdf_seq, _ = self.sample_sdf_and_grad(q_ws, model_id=model_id)
        sdf_seq = sdf_seq - (R + 0.02)
        pen = (-sdf_seq).clamp(min=0.0)

        q_goal = self.goal_state_pos.to(device)
        qg = q_goal.view(1, 1, 2)
        dist_to_goal = (q_seq - qg).norm(dim=2)
        ctrl = (u_safe * u_safe).sum(dim=2)
        ...
        t_star = max(1, H - 1)
        t_idx = torch.arange(H, device=device).view(1, H)
        d0 = (state_q - q_goal).norm().view(1, 1)
        ...
        runtime_cost.add_((u_norm.squeeze(-1) - v_bar.squeeze(-1)).pow(2), alpha=1.0)
        ...
        total_cost = runtime_cost.sum(dim=1) + terminal_cost

        free_mask = torch.ones(B, dtype=torch.bool, device=device)

        return total_cost, q_seq, free_mask
    ...
    def _rollout_double_batch_new2(self, model_id, state_q, us, constraints):
        ...
        device = us.device
        dtype = us.dtype
        state_q_t = state_q.to(device=device, dtype=dtype)
        if self.compile:
            use_amp = self.device in ("cuda", "mps")
            amp_dtype = torch.bfloat16 if self.device == "cuda" else torch.float16
            use_cuda_graph = getattr(self, "_use_cuda_graph", False)
            acc = self._get_rollout_acc(
                model_id=model_id,
                use_amp=use_amp,
                amp_dtype=amp_dtype,
                use_cuda_graph=use_cuda_graph,
            )
            total_cost, q_seq, free_mask = acc.run(state_q_t, us, constraints)
        else:
            total_cost, q_seq, free_mask = self._rollout_full(model_id, state_q_t, us, constraints)
    ...
    def _reverse_diffusion_step(
        ...
        costs, q_seq, free_mask = self._rollout_double_batch_new2(
            model_id=model_id,
            state_q=self.state_inits.q,
            us=actions,
            constraints=self.constraints_ro,
        )
    ...
    def run_inference(
        ...
        for model_id in self.models.keys():
            ...
            horizon = self.models[model_id]['params']['horizon']
            self.constraints_ro = self._pack_constraints_tensor(
                self.soft_constraints,
                M_max=len(self.models),
                N_max=15,
                D=self.robot.q_dim,
